...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  // Queue for map_chunk packets received before dimension registry is ready
  // This prevents chunks from being created with wrong minY values
  let pendingChunks = []
  ...
  // Listen for dimension data ready event to process queued chunks
  bot.on('dimensionDataReady', processPendingChunks)
  ...
  function handleMapChunk (packet) {
    addColumn({
      x: packet.x,
      z: packet.z,
  ...
  bot._client.on('map_chunk', (packet) => {
    // Queue chunks if dimension registry data isn't ready yet
    // This prevents chunks from being created with wrong minY (e.g., 0 instead of -64)
    if (!bot._dimensionDataReady) {
      pendingChunks.push(packet)
      return
    }
    handleMapChunk(packet)
  })
  ...
  async function switchWorld () {
    // Clear any pending chunks from the previous world
    pendingChunks = []
