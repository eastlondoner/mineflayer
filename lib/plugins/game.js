...
function inject (bot, options) {
  ...
  function handleRespawnPacketData (packet) {
    ...
    if (bot.supportFeature('segmentedRegistryCodecData')) { // 1.20.5
      if (typeof packet.dimension === 'number') {
        // Registry may not be loaded yet - try to get dimension name from registry
        const dimName = bot.registry?.dimensionsArray?.[packet.dimension]?.name?.replace('minecraft:', '')
        // If registry lookup fails, try common dimension index mapping (0=overworld, -1=nether, 1=end)
        // This prevents bot.game.dimension from being undefined during the race condition
        bot.game.dimension = dimName ?? dimensionNames[packet.dimension] ?? 'overworld'
      ...
      // Fallback to 'overworld' if dimension index is unknown
      bot.game.dimension = dimensionNames[packet.dimension] ?? 'overworld'
    ...
    if (packet.dimensionCodec) {
      ...
      // Dimension codec in packet means no separate registry_data packet
      bot._dimensionDataReady = true
      bot.emit('dimensionDataReady')
    }
    ...
    if (bot.supportFeature('dimensionDataInCodec')) { // 1.19+
      ...
      // Try to read dimension data from registry if available
      // For 1.20.2+, registry_data packet may not have arrived yet - that's OK because:
      // 1. Chunks are queued in blocks.js until dimensionDataReady fires
      // 2. registry_data handler updates bot.game.minY/height before firing dimensionDataReady
      // 3. So chunks will always be created with correct minY
      const dimData = bot.registry?.dimensionsByName?.[bot.game.dimension]
      if (dimData) {
        bot.game.minY = dimData.minY
        bot.game.height = dimData.height
      }
      // If dimData is not available yet, keep default values (0/256) from lines above
      // The registry_data handler will update these before any chunks are processed
      ...
      // Dimension data directly in packet, no separate registry_data
      bot._dimensionDataReady = true
      bot.emit('dimensionDataReady')
    }
  ...
  // Track when dimension registry data is ready (for chunk queuing)
  bot._dimensionDataReady = false
  bot._dimensionTypeReceived = false
  bot._loginReceived = false

  const brandChannel = getBrandCustomChannelName()
  bot._client.registerChannel(brandChannel, ['string', []])

  // 1.20.2+
  // In 1.20.5+, registry data comes in MULTIPLE packets (banner_pattern, dimension_type, etc.)
  // We must wait for dimension_type specifically before processing chunks
  bot._client.on('registry_data', (packet) => {
    const registryId = packet.id || packet.codec?.type || 'unknown'
    
    bot.registry.loadDimensionCodec(packet.codec || packet)

    // Check if this packet contains dimension_type data
    const isDimensionType = registryId === 'minecraft:dimension_type' || 
                           (packet.codec && packet.codec.value && packet.codec.value['minecraft:dimension_type'])

    if (isDimensionType) {
      // Mark that dimension_type registry data has been received
      bot._dimensionTypeReceived = true
      
      // If login already received (dimension known), update minY/height and emit now
      // This handles the case where login arrives BEFORE dimension_type
      if (bot._loginReceived && bot.game.dimension) {
        const dimData = bot.registry?.dimensionsByName?.[bot.game.dimension]
        if (dimData) {
          bot.game.minY = dimData.minY
          bot.game.height = dimData.height
        }
        if (!bot._dimensionDataReady) {
          bot._dimensionDataReady = true
          bot.emit('dimensionDataReady')
        }
      }
  ...
  bot._client.on('login', (packet) => {
    // NOTE: Do NOT reset _dimensionDataReady here!
    // In 1.20.2+, registry_data packets arrive BEFORE login (in CONFIGURATION phase)
    // Resetting here would cause chunks to be queued forever.

    handleRespawnPacketData(packet.worldState || packet)
    bot._loginReceived = true

    // In 1.20.5+, dimension_type registry arrives BEFORE login, but bot.game.dimension
    // is only set by handleRespawnPacketData above. Now we can look up minY/height.
    if (bot._dimensionTypeReceived && bot.game.dimension && bot.registry?.dimensionsByName) {
      const dimData = bot.registry.dimensionsByName[bot.game.dimension]
      if (dimData) {
        bot.game.minY = dimData.minY
        bot.game.height = dimData.height
      }
      // Emit dimensionDataReady now that we have both dimension_type data AND dimension name
      if (!bot._dimensionDataReady) {
        bot._dimensionDataReady = true
        bot.emit('dimensionDataReady')
      }
